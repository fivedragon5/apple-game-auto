...
# 사과 게임 스크린샷 로드
image = cv2.imread("../image/2025_04_23_20_13_46.png") # 3
# image = cv2.imread("../image/2025_04_23_16_08_03.png") # 100%

# 그레이스케일 변환
gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
cv2.imwrite("./debug_cells/2025_04_23_16_08_03_1.png", gray)

# 색상 반전
inverted = cv2.bitwise_not(gray)
cv2.imwrite("./debug_cells/2025_04_23_16_08_03_inverted.png", inverted)

# 이진화
_, binaryFullImage = cv2.threshold(inverted, 100, 255, cv2.THRESH_BINARY)
cv2.imwrite("./debug_cells/2025_04_23_16_08_03_binary2.png", binaryFullImage)

# OTSU 이진화 적용 (색상 반전된 이미지 사용)
# _, binaryFullImage = cv2.threshold(inverted, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
# cv2.imwrite("./debug_cells/2025_04_23_16_08_03_binary_otsu.png", binaryFullImage)
...
for row in range(ROWS):
    for col in range(COLS):
        ...
        if not text.isdigit():
            ...
            retry_resized = cv2.resize(cell_img, None, fx=3, fy=3, interpolation=cv2.INTER_LINEAR)
            kernel = np.ones((2, 2), np.uint8)

            dilated = cv2.dilate(retry_resized, kernel, iterations=1) # 팽창
